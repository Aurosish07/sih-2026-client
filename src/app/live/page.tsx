...
import type { BasemapKind, ConeCircle } from "@/components/map/CycloneMap";
  ...
  return (
    <div className="min-h-screen overflow-hidden" style={{ background: "linear-gradient(135deg,#eef0f8 0%,#f0f2f7 50%,#edf0f8 100%)" }}>
      ...
      <div className="relative flex min-h-screen flex-col gap-3 px-2 py-2">
        {/* Header */}
        <header className="rounded-2xl border px-4 py-3" style={{ background: "#ffffff", borderColor: "#e4e8f0", boxShadow: "0 4px 20px rgba(99,102,241,0.1)" }}>
          <div className="grid gap-3 lg:grid-cols-[auto_minmax(0,1fr)_auto] lg:items-center">
            <div className="flex items-center gap-3">
              <div className="relative flex h-11 w-11 items-center justify-center rounded-xl text-white text-lg shadow-lg" style={{ background: "linear-gradient(135deg,#6366f1,#10b981)" }}>
                ◉
                <span className="absolute -top-1 -right-1 h-3 w-3 rounded-full border-2 border-white pulse-live" style={{ background: "#10b981" }} />
              </div>
              <div>
                <div className="text-[11px] font-bold uppercase tracking-[0.3em]" style={{ color: "#6366f1" }}>LIVE CYCLONE MONITORING</div>
                <div className="text-sm" style={{ color: "#5a6380" }}>Near-Real-Time · Indian Ocean · NASA GIBS Satellite</div>
              </div>
            </div>
            ...
            <div className="grid gap-2 sm:grid-cols-3 xl:grid-cols-5">
              {[
                ...
                <div key={m.label} className="rounded-xl border px-3 py-2" style={{ background: "#f8f9ff", borderColor: "#e4e8f0", borderLeft: `3px solid ${m.color}` }}>
                  ...
                  <div className="mt-1 text-sm font-semibold" style={{ color: m.color }}>{m.val}</div>
                </div>
              ))}
            </div>
        ...
        <div className="grid flex-1 gap-3 lg:grid-cols-[72px_minmax(0,1fr)_320px]">
          ...
          <section className="relative min-h-[calc(100vh-180px)] overflow-hidden rounded-2xl border shadow-lg" style={{ borderColor: "#e4e8f0", background: "#081018" }}>
            ...
            <div className="absolute top-0 left-0 right-0 z-30 flex items-center gap-3 rounded-t-2xl px-4 py-2 text-xs" style={{ background: "rgba(6,182,212,0.15)", backdropFilter: "blur(8px)", borderBottom: "1px solid rgba(6,182,212,0.3)" }}>
              ...
              <span style={{ color: "#94a3b8" }}>{LAYER_ITEMS.find((l) => l.id === basemap)?.label ?? basemap} · Auto-refresh {refreshMs / 1000}s</span>
            ...
            <div className="absolute inset-0 pt-8">
              {loading && !data ? (
                ...
                <CycloneMapWrapper storm={storm} track={toTrackPoints(data?.historicalTrack ?? [])} forecastTrack={toForecastPoints(data?.forecastTrack ?? [])} cone={toCone(data?.uncertaintyCone)} storms={storm ? [storm] : []} hideOverlays basemap={basemap} />
              )}
            </div>
            ...
            {/* Cyclone info card */}
            {data?.active && storm && (
              <div className="absolute left-3 top-12 z-20 max-w-xs rounded-2xl border p-4 backdrop-blur-xl" style={{ background: "rgba(255,255,255,0.93)", borderColor: "#e4e8f0", boxShadow: "0 8px 32px rgba(99,102,241,0.15)" }}>
            ...
            {/* Legend */}
            <div className="absolute right-3 top-12 z-20 rounded-2xl border px-3 py-2.5 text-[11px] backdrop-blur-xl" style={{ background: "rgba(255,255,255,0.93)", borderColor: "#e4e8f0" }}>
            ...
            {/* Layer switcher */}
            <div className="absolute bottom-3 left-3 z-20 flex items-center gap-2 rounded-2xl border p-2 backdrop-blur-xl" style={{ background: "rgba(255,255,255,0.93)", borderColor: "#e4e8f0" }}>
              {LAYER_ITEMS.map((layer) => (
                <button key={layer.id} type="button" onClick={() => setBasemap(layer.id)}
                  className="rounded-full border px-3 py-1.5 text-xs font-medium transition-all"
                  style={basemap === layer.id ? { background: "#eef2ff", borderColor: "#c7d2fe", color: "#6366f1" } : { background: "#f4f6fb", borderColor: "#e4e8f0", color: "#5a6380" }}>
            ...
            <div className="rounded-2xl border p-4 shadow-sm" style={{ background: "#ffffff", borderColor: "#e4e8f0" }}>
              ...
              <div className="space-y-2 text-sm">
                {[
                  ...
                  { label: "Map layer", val: LAYER_ITEMS.find((l) => l.id === basemap)?.label ?? basemap },
                ].map((r) => (
                  <div key={r.label} className="flex items-center justify-between rounded-xl border px-3 py-2" style={{ background: "#f8f9ff", borderColor: "#e4e8f0" }}>
                    <span className="text-[10px] font-medium uppercase tracking-wider" style={{ color: "#8b95b0" }}>{r.label}</span>
